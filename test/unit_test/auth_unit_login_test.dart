...
import 'auth_unit_login_test.mocks.dart';
...
void main() {
  ...
  test('Login test with valid username and password', () async {
    when(mockAuthUsecase.loginUser('Aasif', 'Aasif123'))
        .thenAnswer((_) => Future.value(const Right(true)));

    await container
        .read(authViewModelProvider.notifier)
        .loginUser(context, 'Aasif', 'Aasif123');
  ...
  test('Login test with invalid username and password', () async {
    when(mockAuthUsecase.loginUser('Aasif', 'Aasif1234'))
        .thenAnswer((_) => Future.value(Left(Failure(error: 'Invalid'))));

    await container
        .read(authViewModelProvider.notifier)
        .loginUser(context, 'Aasif', 'Aasif1234');
